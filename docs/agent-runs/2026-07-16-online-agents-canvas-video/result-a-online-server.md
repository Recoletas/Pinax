...
| `server/realtime/wsHandler.js` | WebSocket 连接生命周期：ready/join/leave/断开/heartbeat/presence 广播/重连 |
...
| `server/__tests__/onlineRoom.test.js` | 8 个单元测试：seq 递增、commandId 去重、host 权限、事件增量读取、snapshot、输入校验 |
...
| `vitest.config.js` | include 增加 `server/**/*.{test,spec}.{js,mjs}` |

## 协议偏差

无。严格按照 README.md 4.1 RoomEvent / 4.3 GenerationJob 冻结契约实现。

- 事件类型：10 种全部实现（room.member.joined/left/updated, chat.message, action.proposed/selected, vote.cast, narrative.requested/completed, runtime.patch.accepted）
...
- `room.member.updated` 事件 — 保留类型，由客户端触发
- WS ping/pong 双向比 `ping { sentAt }` 依赖——service 端 30s 发送 heartbeat pong，客户端 `ping` 回复 `pong` 同时会设心跳时间戳

## 测试命令与结果

```bash
# 单元测试（RoomRegistry + RoomEventStore + validators）
npx vitest run server/__tests__/onlineRoom.test.js
# 结果: 8 passed (8 tests)

# 验证未突破前沿
npx vitest run src/__tests__/legacySnapshot.test.js
# 结果: 16 passed (16 tests) - 0 break

# build
npm run build
# 结果: ✓ built in 5.32s, clean
```

git diff --check: 通过（已有的 advisorTaskService.js blank line 来自其他窗口，非本窗口引入）
