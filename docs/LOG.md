...
- 嵌入图随文字位置移动；四周/紧密图拖到新段落会更新文字锚点，拖到左右半区会切换环绕侧；前后层图片支持页内拖动及横纵位置滑杆。
- 保留 50%-200% 尺寸、完整/铺满、正文间距和重置。版式、锚点、坐标与构图同时写入 NarrativeAsset / MediaAsset，刷新和候选切换后可恢复。
