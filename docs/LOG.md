...
结果摘要：
- 新增 `/comics` 独立漫画制作路由，归入“素材”模块；素材顶栏提供常驻“漫画”入口，抽屉子导航保留“素材库 / 漫画制作”，画布只保留关系画布。
- 独立工作区使用左侧漫画页列表、中间整页预览、右侧当前格检查器；新建和切换漫画页不再依赖素材页当前选中项。
- 每个漫画格单独选择一条素材，绑定写入既有 `ComicPanel.continuityRefs`；页面 `sourceRefs` 只负责汇总各格来源，不再反向决定整页属于哪条素材。
- 格级素材驱动该格的来源标题、正文上下文、生图提示和 MediaAsset 归档；空画面在选素材时提取首个叙事瞬间作为可编辑起点，已有画面描述不会被覆盖。
- 批量补齐在独立模式下要求每个未完成格都已选择素材并有画面描述，避免缺少来源的格被静默生成；刷新后每格绑定保持不变。
- 素材页删除“漫画制作”副工作台、中央漫画替换预览和相关组件状态，只保留相关素材与插画生成；“相关素材”下方重复显示当前素材名的蓝色缩略条及其状态逻辑一并删除。

验证：`npm run verify:full`，核心 23 files / 188 tests、视觉 1 file / 12 tests，Vite/VitePress build 和 `git diff --check`；总量 200。浏览器回归覆盖素材顶栏入口、两格绑定不同素材、刷新恢复、素材页旧副工作台与重复缩略条移除、900/1440px 无横向溢出和无控制台错误。未启动 dev server。

## 2026-07-19 - 漫画单幅生成与文字层分离

状态：完成漫画生成纠偏和 M6 文字排版的基础切片；高级气泡样式、尾巴、字体和排版质检仍待后续阶段。
