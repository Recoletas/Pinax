...
- 欢迎页右栏不再展示泛化的“新建、请 AI、备份”功能说明，改成可直接照做的三步创作回路：先落正文，在「角色 / 当前场」让人物入场，再用「推演」试走岔路；末尾把刷新确认和导出备份收成一个明确结束动作。
- 保持 Pinax 现有安静编辑视觉，只用排版、细线和步骤关系强化层级；没有增加弹窗、徽章、悬浮导览或进入稿面后的遮挡。旧 UTF-8-only 文案同步移除，模型不可用时仍能直达连接检查。
- Web beta Gate 增加关键指引文案与旧文案消失断言；1440、390 以及最近书稿状态均完成截图复核。最终 `verify:full` 为 20/20 文件、200/200 用例、双 build 与 diff check 全绿。
