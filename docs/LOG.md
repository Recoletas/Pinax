...
结果摘要：
- 新增 `/comics` 独立漫画制作路由，归入“画布”模块；画布顶栏提供常驻“漫画”入口，抽屉子导航同时保留“关系画布 / 漫画制作”。
...
- 素材页删除“漫画制作”副工作台、中央漫画替换预览和相关组件状态，只保留相关素材与插画生成。

验证：`npm run verify:full`，核心 23 files / 188 tests、视觉 1 file / 12 tests，Vite/VitePress build 和 `git diff --check`；总量 200。浏览器回归覆盖画布顶栏入口、两格绑定不同素材、刷新恢复、素材页旧入口移除、900/1440px 无横向溢出和无控制台错误。未启动 dev server。
