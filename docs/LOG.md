...
结果摘要：
- 生成参数继续留在右侧副工作台；中央主阅读区负责生成后的图片展示与构图，不再向生成表单增加重复按钮。
- 插画可直接拖动调整位置，支持 50%-200% 缩放、“完整 / 铺满”显示方式、重置，以及方向键微调；窄屏工具栏会自动换行。
- 构图参数同时写入 NarrativeAsset 与 MediaAsset，旧图片自动使用兼容默认值；生成候选切换回来或页面刷新后仍能恢复已保存的位置、大小和显示方式。
- 沿用档案纸张、蓝灰 token、低圆角与轻分隔，不引入独立浮层或嵌套卡片；未启动 dev server。
