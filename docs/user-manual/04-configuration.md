...
`poetry_idea_tree_v2` / `poetry_idea_positions_v2` / `poetry_adapt_profile_v2` / `poetry_graph_edges_v1` / `poetry_imagery_groups_v1` / `poetry_snapshots_v1` / `poetry_image_library_v1` —— 历史遗留键，主要来自旧画布形态；当前主流程一般不直接依赖它们。
...
注意：**完整导出会包含 `apiSettings` 和 `mem0_settings`，也就是你的 LLM / Mem0 key**。如果只是想迁移内容数据，建议先删掉这两个键，或者用下面这个“去敏版”：

```js
const safeData = Object.fromEntries(
  Object.entries(localStorage).filter(([key]) => !['apiSettings', 'mem0_settings'].includes(key))
)
copy(JSON.stringify(safeData))
```
