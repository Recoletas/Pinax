...
- 2026-07-19：漫画制作保留独立 `/comics` 工作区，但入口回到素材页副阅读台，与“相关素材 / 插画生成”并列，不再占用素材模块顶栏。漫画页左侧复用素材抽屉的信息结构，点击素材直接绑定当前格；漫画页序列移到中央画布顶部，右侧继续制作当前格。每格通过既有 `continuityRefs` 单独选择素材，页级 `sourceRefs` 自动汇总；素材页旧漫画副工作台和“相关素材”下方无操作价值的当前素材缩略条均已删除。`verify:full` 通过核心 188 + 视觉 12、Vite/VitePress build 和 diff check；900/1440px 浏览器绑定/刷新回归通过，未启动 dev server。
