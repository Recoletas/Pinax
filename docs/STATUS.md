...
- 2026-07-19：漫画制作从素材页当前选中项中迁出，新增 `/comics` 独立三栏工作区，并作为素材模块的独立子入口；左侧管理漫画页，中间整页预览，右侧制作当前格。每格通过既有 `continuityRefs` 单独选择一条素材，页级 `sourceRefs` 自动汇总，生图上下文和归档来源按格读取；批量补齐会阻止未选素材或无画面的格。素材页旧漫画副工作台和“相关素材”下方无操作价值的当前素材缩略条均已删除。`verify:full` 通过核心 188 + 视觉 12、Vite/VitePress build 和 diff check；900/1440px 浏览器绑定/刷新回归通过，未启动 dev server。
