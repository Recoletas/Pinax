...
2026-09-18 新一夜进入**调研完成、待执行**：[记忆/跑团/Agent 成熟度夜间计划](./plan/nightly-20260918-runtime-maturity.md)，依据[固定上游源码比较](./engineering/maturity-research-20260918.md)。按用户反馈改为持续执行队列：首批 G 验收后自动推进 M/R/T 共 36 项，覆盖时序协调、角色知识、AI 主持/同伴、跨会话战役和工具治理；估时不是收工上限，单项阻塞切换可执行任务。成熟开源代码优先复用，各批验收后继续，不将最小闭环当完成。未启动 worker、合并或调用真实模型，当前 UI WIP 保留。
