...
素材页保留当前三栏总体布局，但漫画入口改名为“漫画制作”，进入后不继续复用插画表单：

- **左侧/底部页序列**：页缩略图、页码、阶段完成度、stale/失败标记；支持新建、复制、重排和页漫/条漫查看；
...
- `Notes.vue`：只持有选中素材、当前漫画项目和跨工作区事件，不持有制作算法。
...
当前状态（2026-07-19）：共享 provider、MediaAsset 和现有漫画页制作字段已贯通；M0-M1 完成。素材副工作台已按“页面规划 / 分格制作”收口，页级方向、画风/连续性、格级 beat、镜头和制作状态均有可见编辑入口，空白页真实支持 4/6 格。逐格生图已纠正为结合素材、页规则、当前镜头和上一格锚点的单幅无文字画面；支持参考图的 provider 会复用上一格成图，MiniMax Image 按能力降级为文本连续性。M6 已完成基础文字对象、拖动/缩放、整页预览和 PNG 合成，但尾巴、字体、溢出/遮挡检查及完整出版导出尚未完成。这些都不等于 M2；下一步仍是多页改编候选与从角色、地点、已有插画建立的可审阅视觉圣经。
