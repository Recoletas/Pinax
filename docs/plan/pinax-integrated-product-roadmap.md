...
- `/comics` 是独立漫画制作工作区，不隶属于素材页当前选中项；中央主区是可选择格框和文字对象的页面画布，右侧是当前页/当前格的阶段检查器，生成配置不进入中央画布；
...
漫画制作使用独立 `/comics` 三栏工作区，归入“画布”模块，不继续复用素材页的插画副工作台：

- **左侧页序列**：页缩略图、页码、阶段完成度、stale/失败标记；支持新建、复制、重排和页漫/条漫查看；
- **中央页面画布**：始终显示真实页面或条漫画布；顶部模式为 `分镜 / 制作 / 文字 / 成品`；格框、构图控制点和气泡只在对应模式显示；
- **右侧阶段检查器**：显示当前页或当前格的叙事 beat、导演参数、参考绑定、阶段链、候选、生成/上传/采纳/回退动作；
...
当前状态（2026-07-19）：共享 provider、MediaAsset 和现有漫画页制作字段已贯通；M0-M1 完成。漫画已从单条素材的副工作台迁入独立 `/comics` 工作区，页面列表、整页预览和当前格检查器分栏显示；每格通过 `continuityRefs` 选择自己的主要素材，页级来源仅做汇总。页级方向、画风/连续性、格级 beat、镜头和制作状态均有可见编辑入口，空白页真实支持 4/6 格。逐格生图已纠正为结合格级素材、页规则、当前镜头和上一格锚点的单幅无文字画面；支持参考图的 provider 会复用上一格成图，MiniMax Image 按能力降级为文本连续性。M6 已完成基础文字对象、拖动/缩放、整页预览和 PNG 合成，但尾巴、字体、溢出/遮挡检查及完整出版导出尚未完成。这些都不等于 M2；下一步仍是多页改编候选与从角色、地点、已有插画建立的可审阅视觉圣经。
