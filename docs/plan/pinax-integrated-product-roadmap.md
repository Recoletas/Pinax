...
当前状态（2026-07-19）：共享 provider、MediaAsset 和现有漫画页制作字段已贯通；M0-M1 完成。漫画已从单条素材的副工作台迁入独立 `/comics` 工作区，入口回到素材页副阅读台；左侧素材抽屉、中央页签/整页预览和右侧当前格检查器分栏显示。每格通过 `continuityRefs` 选择自己的主要素材，页级来源仅做汇总。页级方向、画风/连续性、格级 beat、镜头和制作状态均有可见编辑入口，空白页真实支持 4/6 格。逐格生图已纠正为结合格级素材、页规则、当前镜头和上一格锚点的单幅无文字画面；支持参考图的 provider 会复用上一格成图，MiniMax Image 按能力降级为文本连续性。M6 已完成基础文字对象、拖动/缩放、整页预览和 PNG 合成，但尾巴、字体、溢出/遮挡检查及完整出版导出尚未完成。这些都不等于 M2；下一步仍是多页改编候选与从角色、地点、已有插画建立的可审阅视觉圣经。
