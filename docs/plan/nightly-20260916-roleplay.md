...
> 状态：**调研完成、待执行任务书**。本文件不表示运行时代码已实现，不表示真实模型、联机或用户视觉验收已通过。属于 G1.4 / G4.6，体验与创作主线不另起门户。C 线只负责单人跑团纵切；记忆历史归 A 线，漫画归 B 线，共享接线归集成 owner O。任务 ID 保留 R00–R29（Roleplay），不代表第四条工作线。
