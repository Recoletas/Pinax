...
- `ProseEssay` 的直接导出现在会写入统一文档并先做结构校验，`PoetryLab` 分镜视图的 Markdown 导出也已接入统一文档，体验和写作导出则统一走校验保存入口。
...
目标：模式切换必须表达“工作流视图”，不能只刷新标签而复用同一套文本假装已经转换。

当前决策：

- 诗歌页不再使用“写作/编导”标签，改为“意象/分镜”。
- 散文页不再使用“写作/编导”标签，改为“卡片/分镜”。
- 切到分镜侧时，主动作应是“从当前意象树/卡片生成分镜草稿”，而不是把原文本换个标签展示。
- 没有分镜版本时，应显示空态和生成入口；已有版本时，应展示统一 `StoryboardVersion` 的版本卡片、镜头列表、校验状态和下载入口。
- 诗歌/散文保留为素材输入和结构生成入口，最终分镜历史、校验和导出继续向分镜工作台收口。

后续迁移：

1. 先完成文案和分段控件替换，避免旧开关误导用户。
2. 再拆分来源内容与分镜草稿状态，避免模式切换污染原始卡片/意象树。
3. 最后移除页面私有直接下载路径，统一走“保存版本 -> 用户确认 -> 下载”。
