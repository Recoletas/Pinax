...
> **定位已调整：本文件现在是迁移期执行骨架，不再独占产品方向。**
>
> Pinax 的目标外壳已经切向 [character-driven-arc.md](./character-driven-arc.md)；但在实现层，用户先选择世界、进入冒险、沉淀剧情、写回作品，这条 `playable-worldbook` 主链仍然是最可复用的 runtime / content 骨架。

## 1. 执行定位

这个文件回答的不是“最终产品看起来像什么”，而是：

- 当前底层主链如何继续推进；
- 哪些 runtime / content / trigger 工作对新方向仍然有价值；
- 在 UI 壳层换代时，哪些实现骨架不能丢。
...
短期策略依然是“沉浸世界观 + AI GM 控制”。只是这条链路接下来会被角色化 GM 外壳重新包装，而不是继续停留在旧工具化视觉里。

补充边界：

- 首屏海报构图、角色多 pose、雨滴/楔形选项语法、C5 cinematic 切换风格现在以 [character-driven-arc.md](./character-driven-arc.md) 为准；
- 本文件只负责保证 `选择世界 -> 进入冒险 -> 写回作品` 的 runtime 连续性，不负责定义最终 hero 长相或动画语言。
...
目标：让所有文档能区分清楚“产品方向”和“执行骨架”，避免再把旧工具集合叙事或单一路线图误读成全部真相。

交付：

- `docs/STATUS.md` 写清当前阶段、下一步和阻塞点。
- `docs/PLAN.md` 明确“角色化方向已采纳，playable-worldbook 作为执行骨架保留”。
- `docs/plan/playable-worldbook-roadmap.md` 作为本专题路线图。
- `README.md` / `docs/README.md` 同时给出方向文档和执行骨架入口。

验收：

- 新会话读 `docs/PLAN.md`、方向文档和本文件，即可知道“往哪走”和“底层先做什么”。
...
状态：首轮已落地。**从现在起只保留行为连续性价值，不再继续把旧壳层 polish 成最终方向。**

目标：保证 runtime 入口连续性成立；视觉外壳的大改已经转移到 `character-driven-arc`。
...
- 如果继续在旧任务板外壳上深投视觉，会和角色化迁移发生直接冲突。
...
Stage 1 后续任何 UI 打磨都遵守以下补充原则：

- **开场钩子是显式意图，不是装饰文案。**
  用户在首屏或快速导入页选中的开场行动，必须被当作后续体验链路的真实输入，而不是只影响卡面高亮。
- **跨页面连续性优先。**
  `WelcomeView -> WorldBookQuickImport -> Experience` 必须能回显并继承同一条钩子；只有用户主动改选、清除或意图过期时，才允许切换。
- **CTA 优先级固定。**
  任何“开始冒险 / 进入这个世界”类无参主按钮，都必须遵守：
  `显式点击的 action > 当前携带的 intent > 默认 fallback`。
  不允许静默把用户已选择的钩子覆盖成 `hooks[0]`。
- **预选态与激活态分离。**
  “从上一页带来的预选状态”和“当前页刚点击的激活状态”不是一回事，不能复用同一个 class、flag 或语义名混写。
- **生命周期要可清除、可过期，但不要把 helper 永久收窄成 preset-only。**
  当前主链路可以通过 preset 带意图进入体验，但相关 helper 仍应保留向“非预设世界书入口”扩展的余地；不要把 API 契约永久绑定在 preset 场景上。
- **测试断言以行为为准。**
  优先验证按钮数量、预选样式、banner 回响、override/clear/stale 路径和最终进入体验时使用的 action；避免用源码字符串 grep 代替行为断言。

Review gate：

- 跑 UI 契约测试、视觉测试、全量测试和构建；确认入口行为没有破坏旧路由。
- 不把通过这一步误读为“旧首页就是未来方向”。
- 如果入口行为连续性和新的海报构图发生冲突，以连续性为最小底线、以 `character-driven` 方向为最终壳层归属；不要再把旧任务板补成最终产品。
