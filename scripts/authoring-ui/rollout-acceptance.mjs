...
check('工具顺序＝撤销/重做|排版/取名|专注|查找（实用工具，格式移选区浮条）', order === '撤销|重做|排版|取名|专注|查找', order)
...
await page.waitForTimeout(300)
await page.fill('.find-replace-bar .find-input:not(.fr-replace-input)', '税务所')
await page.keyboard.press('Enter')
await page.waitForTimeout(400)
const findCount = await page.evaluate(() => document.querySelector('.find-count')?.textContent || '')
check('命中计数显示（税务所）', /\d+\/\d+/.test(findCount), findCount)
await page.fill('.find-replace-bar .find-input:not(.fr-replace-input)', '不存在的词组')
await page.keyboard.press('Enter')
await page.waitForTimeout(900)
check('零结果显示无匹配', (await page.evaluate(() => document.querySelector('.find-count')?.textContent)) === '无匹配')
await page.fill('.find-replace-bar .find-input:not(.fr-replace-input)', '黄铜钥匙')
await page.keyboard.press('Enter')
await page.fill('.find-replace-bar .fr-replace-input', '黄铜锁钥')
await page.locator('.find-replace-bar button', { hasText: '单处' }).click()
await page.waitForTimeout(400)
check('单处替换生效', await page.evaluate(() => document.querySelector('.ProseMirror').textContent.includes('黄铜锁钥')))
await page.keyboard.press('Control+z')
await page.waitForTimeout(300)
check('替换可撤销', !(await page.evaluate(() => document.querySelector('.ProseMirror').textContent.includes('黄铜锁钥'))))
await page.locator('.find-replace-bar .fr-close').click()
await page.waitForTimeout(200)
check('关闭查找条', await page.evaluate(() => !document.querySelector('.find-replace-bar')))
record('TB-09', '查找/计数/零结果/单处替换/撤销')
