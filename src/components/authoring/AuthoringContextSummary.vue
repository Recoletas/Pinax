<template>
  <section v-if="manifest || selectedReferences.length" class="authoring-context-summary" data-test="context-summary" :data-context-mode="mode" :aria-label="tr(mode === 'actual' ? '生成后实际参考' : '生成前上下文预检')">
    <details :open="openByDefault">
      <summary>
        <strong>{{ tr(mode === 'actual' ? '实际参考' : '参考范围') }}</strong>
        <span>{{ summaryText }}</span>
      </summary>
      <div class="authoring-context-summary__body">
        <section v-for="group in visibleGroups" :key="group.id">
          <h4>{{ tr(group.label) }} <span>{{ group.items.length }}</span></h4>
          <ul>
            <li v-for="item in group.items" :key="item.id">
              <span><strong>{{ item.label }}</strong><small>{{ item.reason }}</small></span>
              <span class="authoring-context-summary__state">{{ tr(item.state) }}</span>
              <button v-if="mode === 'planned' && item.candidateId" type="button" @click="$emit('exclude', item.candidateId)">{{ tr('本次排除') }}</button>
            </li>
          </ul>
        </section>
        <p v-if="loading">{{ tr('正在按当前落笔处核对参考…') }}</p>
        <p v-else-if="error">{{ tr('暂时无法完成预检；提交时仍会重新冻结并校验。') }}</p>
      </div>
    </details>
  </section>
</template>
...
function exclusionReason(reason) {
  if (String(reason).startsWith('budget-')) return tr('本轮篇幅有限，未送入模型')
  if (reason === 'conflict-set-loser') return tr('与更可靠的来源冲突')
  if (reason === 'author-removed-for-run') return tr('已由作者从本次推演移除')
  if (['source-revision-changed', 'revision-missing-fail-closed', 'source-missing', 'source-stale'].includes(reason)) return tr('来源版本已经变化或不可读取')
  return tr('与本次落笔处关联不足')
}

function displayLabel(label, kind) {
  const value = String(label || '').trim()
  // These labels are generated by the context reader, unlike worldbook and
  // outline titles, which belong to the author and must remain untouched.
  if (kind === 'manuscript-unit') {
    const previous = /^前文片段 (\d+)$/.exec(value)
    if (previous) return tr('前文片段 {count}', { count: previous[1] })
    if (['待重写文本块', '当前落笔处'].includes(value)) return tr(value)
  }
  if (kind === 'scene-projection' && value === '当前场') return tr('当前场')
  if (kind === 'memory') {
    const category = ({
      'plot-event': '剧情记忆',
    ...
    return category ? tr(category) : value && !value.includes('-') ? value : tr('相关记忆')
  }
  return value || kindFallback(kind)
}

function inclusionReason(reason) {
  const value = String(reason || '')
  const previous = /^目标前第 (\d+) 个写作单元$/.exec(value)
  if (previous) return tr('目标前第 {count} 个写作单元', { count: previous[1] })
  const worldbookMatch = /^世界书命中（(.+)）$/.exec(value)
  if (worldbookMatch) return tr('世界书命中（{reason}）', { reason: worldbookMatch[1] })
  return tr(value || '与当前落笔处相关')
}
...
const manifestItems = computed(() => {
  const blocks = (props.manifest?.blocks || []).map((block) => ({
    ...
    reason: inclusionReason(block.reason),
...
const selectedFallback = computed(() => props.selectedReferences.map((item) => ({
  ...
  label: item.label,
  reason: item.selectionStatus === 'ready' ? tr('作者选中的本次{role}', { role: roleLabel(item.usageRole) }) : tr('需要核对来源'),
...
const summaryText = computed(() => visibleGroups.value.flatMap((group) => {
  ...
  return count > 0 ? [`${tr(group.shortLabel)} ${count}`] : []
}).join(' · ') || tr('提交时核对'))
...
function roleLabel(role) {
  return tr(({ fact: '事实', intent: '意图', inspiration: '灵感' })[role] || '灵感')
}
</script>
...
.authoring-context-summary summary strong { flex: 0 0 auto; color: var(--text-primary); font-size: 12px; }
...
.authoring-context-summary li small { white-space: normal; line-height: 1.45; overflow-wrap: break-word; }
.authoring-context-summary__state { flex: 0 1 auto; max-width: 7.5em; color: var(--archive-olive-strong); }
