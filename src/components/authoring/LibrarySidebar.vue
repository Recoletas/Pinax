...
const projectTools = [
  { name: 'settings-structured', label: '人物与设定', icon: 'settings' },
  { name: 'settings-world-map', label: '世界地图', icon: 'compass' },
  { name: 'materials', label: '灵感素材', icon: 'archive' },
  { name: 'prose-essay', label: '画布与分镜', icon: 'storyboard' },
  { name: 'comics', label: '漫画制作', icon: 'image' }
]
...
.library-sidebar__project select { width: 100%; min-height: 36px; margin-top: 5px; padding: 6px; border: 1px solid var(--archive-paper-strong); border-radius: 4px; color: var(--archive-ink); background: var(--archive-paper-soft); font: inherit; font-size: 13px; }
