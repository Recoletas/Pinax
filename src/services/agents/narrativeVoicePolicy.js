...
export function buildNarrativeVoiceContract() {
  return [
    ...
    '每一轮正文必须让至少一项变化落地：信息被确认/否定、关系改变、目标进展、障碍生效或行动产生后果；没有变化只有描写的正文不合格。',
    '环境、气氛、光影、声响和身体反应只有在改变判断、动作、信息或局势时才保留；不为长度堆景物，不按视觉/听觉/嗅觉逐项罗列。',
    '最近正文已写过的物件、声响、小动作不因"文采"再写一次；只有它直接造成新后果时才回收。',
    '情绪落在停顿、措辞和动作上，不替读者概括"紧张、复杂、震惊"等结论。',
    '角色台词回应眼前的人和事，保留各自目的与信息边界；允许停顿、打断、答非所问，不让角色借台词朗读设定；用神态描写回避回答不合格。',
