/**
 * 自包含 SHA-256（FIPS 180-4）纯实现。
 *
 * 为什么不用 crypto.subtle：jsdom 测试环境不保证 subtle，浏览器/Node/测试三处
 * 必须产出完全一致的哈希（manifest 校验是整包拒绝的判据）。
 * 输入 Uint8Array，输出小写 hex。已在与 Node crypto 的对拍中验证（见 backupExport.test.js）。
 */
