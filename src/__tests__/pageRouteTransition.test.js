...
describe('page route transition contract', () => {
  ...
  it('keeps page-route transition timing and reduced-motion rules in the global stylesheet', () => {
    ...
    expect(mainCss).toMatch(/\.page-route-enter-active\s*\{[^}]*transition:\s*opacity 0\.28s cubic-bezier\(0\.22,\s*1,\s*0\.36,\s*1\),\s*transform 0\.28s cubic-bezier\(0\.22,\s*1,\s*0\.36,\s*1\);[^}]*will-change:\s*opacity,\s*transform;/s)
    expect(mainCss).toMatch(/\.page-route-leave-active\s*\{[^}]*transition:\s*opacity 0\.16s cubic-bezier\(0\.22,\s*1,\s*0\.36,\s*1\),\s*transform 0\.16s cubic-bezier\(0\.22,\s*1,\s*0\.36,\s*1\);/s)
    expect(mainCss).toMatch(/\.page-route-enter-from\s*\{[^}]*opacity:\s*0;[^}]*transform:\s*translateY\(8px\) scale\(0\.995\);/s)
    expect(mainCss).toMatch(/\.page-route-leave-to\s*\{[^}]*opacity:\s*0;[^}]*transform:\s*translateY\(-6px\);/s)
