...
describe('authoring block composer', () => {
  it('owns a frozen target and submits a canonical turn', async () => {
    ...
    expect(wrapper.find('textarea').attributes('aria-label')).toBe('推演要求')
    expect(wrapper.find('.authoring-block-composer__more').element.open).toBe(false)
    expect(wrapper.find('.authoring-block-composer__more summary').text()).toContain('推演选项')
